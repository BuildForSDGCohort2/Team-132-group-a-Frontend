...
MainController.prototype.run = function(mysql) {
	...
	/*_userManager.getUser('david.sibato@to-sala.com', function(found, user){
		if(found){
			_src.FileUtils.logger(user.getNom());
		}
	});*/
...
_app.post('/community/create.do', function(req, res){
	if(req.session.user){
	    ...
	    _communityManager.addCommunity(req.session.userObject.id, name, desc, tags, function(saved, communityId){
	    	if(saved){
	    		_communityManager.addFollower(req.session.userObject.id, communityId, 1, function (ok) {
					if(ok){
                        res.send({tag:'success', community: communityId});
					}
                });
	    	}else{
